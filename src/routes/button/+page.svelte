<script lang="ts">
    ...
    let rounded: SizeStyle = $state("md");
</script>

<div class="w-full h-screen flex mt-20">
    <div class="w-3/4 flex-center">
        <Button color={color} {variant} {size} {rounded}>Button</Button>
    </div>
    ...
    <div class="w-1/4 p-4 flex flex-col gap-2">
        ...
        <div class="w-full flex flex-wrap">
            {#each Object.keys(colorStyleParts) as colorKey}
                <Button size={40} rounded="none" color={colorKey as ColorStyle} onclick={() => color = colorKey as ColorStyle}></Button>
            {/each}
        </div>
        <FloatingSelect divClass="w-full" options={sizeOptions} bind:value={size}>Size</FloatingSelect>
        <FloatingSelect divClass="w-full" options={sizeOptions} bind:value={rounded}>Rounded</FloatingSelect>
    </div>
</div>